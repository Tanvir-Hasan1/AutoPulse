...
export default function TaxToken() {
  ...
  const handleTaxTokenPress = async () => {
    ...
    try {
      ...
      const data = await api.get(`/tax-token/info/${selectedBikeId}`);
      if (!data || !data.taxToken) {
        throw new Error("No tax token found for this bike");
      }

      const contentType = data.taxToken.contentType;
      if (contentType && contentType.includes("pdf")) {
        ...
        // Fetch image as blob
        const blob = await api.get(`/tax-token/download/${selectedBikeId}`, {
          responseType: "blob",
        });
